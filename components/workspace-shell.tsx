...
export function WorkspaceShell({ user, workspace, workspaceId, project, projects, documents: initialDocuments, initialActiveId, databases: initialDatabases, records, myTasks, teamMembers: initialTeamMembers }: { user: { id: string; name: string; role: string; avatarEmoji?: string | null }; workspace: string; workspaceId: string; project: { id: string; name: string; code: string }; projects: Array<{ id: string; name: string; code: string }>; documents: DocumentItem[]; initialActiveId?: string; databases: DatabaseData[]; records: RecordData; myTasks: MyTask[]; teamMembers: TeamMember[] }) {
  ...
  return <div className="app">
    ...
    <aside className="rightbar">
      ...
      <MyWorkPanel tasks={myTasks} editable={currentUser.role !== "VIEWER"} currentProjectId={project.id} onOpenTasks={(projectId) => { if (projectId !== project.id) return window.location.assign(`/?project=${encodeURIComponent(projectId)}`); setModule("tasks"); setActiveDatabaseId(""); setShowSettings(false); setShowTeam(false); setShowRecycle(false); }} />
