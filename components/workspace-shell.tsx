...
export function WorkspaceShell({ user, workspace, workspaceId, project, projects, documents: initialDocuments, initialActiveId, initialModule, initialSelectedTaskId, databases: initialDatabases, records, myTasks, teamMembers: initialTeamMembers }: { user: { id: string; name: string; role: string; avatarEmoji?: string | null }; workspace: string; workspaceId: string; project: { id: string; name: string; code: string }; projects: Array<{ id: string; name: string; code: string }>; documents: DocumentItem[]; initialActiveId?: string; initialModule?: WorkspaceModule; initialSelectedTaskId?: string; databases: DatabaseData[]; records: RecordData; myTasks: MyTask[]; teamMembers: TeamMember[] }) {
  ...
  async function createDocument(parentId?: string, templateId?: string, providedTitle?: string) {
    const title = providedTitle || window.prompt("新文件名稱"); if (!title?.trim()) return;
  ...
  return <div className="app">
    <aside className="sidebar">
      ...
      <button className="create-document" onClick={() => createDocument()}><span>＋</span> 新增頁面</button><button className="create-document template-create" onClick={() => setTemplatePicker(true)}><span>◇</span> 從模板建立</button>
