...
type RecordData = {
  ...
  >;
  issues: Array<
    ModuleRecord & { key: string; title: string; status: string; severity: number }
  >;
  bom: Array<
    ModuleRecord & {
      partNumber: string;
  ...
  >;
  tests: Array<
    ModuleRecord & { title: string; outcome: string; operator: string | null }
  >;
};
...
function DocumentTree({
  ...
  const byParent = useMemo(
    () =>
      documents.reduce<Record<string, DocumentItem[]>>((acc, item) => {
  ...
  const databasesByDocument = useMemo(
    () =>
      databases
        .filter((database) => database.parentDocumentId)
        .reduce<Record<string, DatabaseData[]>>((acc, database) => {
          (acc[database.parentDocumentId || ""] ||= []).push(database);
          return acc;
        }, {}),
    [databases],
  );
  ...
  const render = (parentId: string | null, depth = 0): React.ReactNode =>
    (byParent[parentId || "root"] || []).map((item) => {
      const hasChildren = Boolean(
        byParent[item.id]?.length || databasesByDocument[item.id]?.length,
      );
      return (
        <div className="tree-item" key={item.id}>
          <div
            className={`tree-row ${dropTarget === item.id ? "drag-target" : ""}`}
            draggable
            onContextMenu={(event) => {
              ...
              const placement =
                event.clientY < rect.top + rect.height * 0.28
                  ...
                  : event.clientY > rect.bottom - rect.height * 0.28
                    ? "after"
                    : "inside";
              onMove(source, item.id, placement);
              setDraggedId(null);
              setDropTarget(null);
            }}
          >
            ...
            >
              <span className="tree-rail" style={{ marginLeft: depth * 14 }}>
                {item.icon || "📄"}
              </span>
              <span>{item.title}</span>
            </button>
            <button
              className="tree-subpage"
              aria-label={`在 ${item.title} 下新增子頁面`}
          ...
          {!collapsed.has(item.id) && (
            <>
              ...
              {(databasesByDocument[item.id] || []).map((database) => (
                <button
                  key={database.id}
                  className={
                    activeDatabaseId === database.id
                      ? "tree-database active"
                      : "tree-database"
                  }
                  style={{ paddingLeft: 31 + (depth + 1) * 14 }}
...
export function WorkspaceShell({
  ...
  useEffect(() => {
    ...
    void fetch("/api/my-tasks?take=50", { cache: "no-store" })
      .then(async (response) =>
        response.ok
          ? (response.json() as Promise<{ tasks: MyTask[]; nextCursor: string | null }>)
          : null,
      )
      .then((result) => {
        if (!cancelled && result) {
          setPersonalTasks(result.tasks);
          setNextPersonalTaskCursor(result.nextCursor);
        }
      })
      .finally(() => !cancelled && setLoadingPersonalTasks(false));
    return () => {
      cancelled = true;
    };
  }, []);
  async function loadMorePersonalTasks() {
    if (!nextPersonalTaskCursor || loadingPersonalTasks) return;
    setLoadingPersonalTasks(true);
    try {
      const response = await fetch(
        `/api/my-tasks?take=50&cursor=${encodeURIComponent(nextPersonalTaskCursor)}`,
        { cache: "no-store" },
      );
      ...
      setPersonalTasks((current) => [...current, ...(result.tasks || [])]);
      setNextPersonalTaskCursor(result.nextCursor || null);
    } catch {
      notify("無法載入更多任務", "請稍後再試。");
    } finally {
  ...
  useEffect(() => {
    ...
    if (!document || document.content !== undefined) return;
    let cancelled = false;
    void fetch(`/api/documents/${document.id}`, { cache: "no-store" })
      .then(async (response) =>
        response.ok
          ? (response.json() as Promise<Pick<DocumentItem, "content" | "markdown">>)
          : null,
      )
      .then((loaded) => {
        if (!cancelled && loaded)
          setDocuments((current) =>
            current.map((item) =>
              item.id === document.id ? { ...item, ...loaded } : item,
            ),
          );
      });
    return () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetch(`/api/databases/${activeDatabase.id}`, { cache: "no-store" })
      .then(async (response) =>
        response.ok ? (response.json() as Promise<DatabaseData>) : null,
      )
      .then((database) => {
        if (!cancelled && database)
          setDatabases((current) =>
            current.map((item) =>
              item.id === database.id ? { ...database, rows: [] } : item,
            ),
          );
      });
    return () => {
  ...
  async function createDatabase(name: string, parentDocumentId?: string) {
    if (!name.trim()) return;
    const response = await fetch(`/api/projects/${project.id}/databases`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: name.trim(),
  ...
  async function deleteDocument(id: string) {
    ...
    if (!response.ok) return notify("無法刪除文件", "請確認你的編輯權限後再試一次。");
  ...
  async function moveDocument(
    ...
    const siblings = documents
      .filter((item) => item.parentId === parentId && item.id !== draggedId)
  ...
  const requestCreateDocument = (parentId?: string) =>
    setDialog({
      title: parentId ? "新增子頁面" : "新增頁面",
      description: "建立後可在文件內繼續編輯內容與設定圖標。",
      initialValue: "",
      confirmLabel: "建立頁面",
      onConfirm: (title) => void createDocument(title, parentId),
    });
  const requestCreateDatabase = (parentDocumentId?: string) =>
    setDialog({
      title: parentDocumentId ? "在此頁面下新增資料庫" : "新增資料庫",
      description: parentDocumentId
        ? "資料庫會與子頁面並列在目前文件下方。"
        : "資料庫可用於追蹤任務、物料、測試與自訂資料。",
      initialValue: "專案追蹤表",
      confirmLabel: "建立資料庫",
      onConfirm: (name) => void createDatabase(name, parentDocumentId),
    });
  const requestDeleteDocument = (id: string) => {
    ...
    setDialog({
      title: "移至回收桶？",
      description: `「${document.title}」及其子頁面會移至回收桶，之後仍可還原。`,
      confirmLabel: "移至回收桶",
      destructive: true,
      onConfirm: () => void deleteDocument(id),
    });
  };
  async function loadMoreDocuments() {
    if (!nextDocumentCursor) return;
    const response = await fetch(
      `/api/projects/${project.id}/documents?take=50&cursor=${encodeURIComponent(nextDocumentCursor)}`,
      { cache: "no-store" },
    );
    const result = (await response.json()) as {
      documents?: DocumentItem[];
    ...
    <div className="app">
      <aside className="sidebar">
        <div className="brand-lockup">
          ...
          <div>
            ...
            <span className="brand-subtitle">任務控制台</span>
          </div>
        </div>
        <section className="project-card">
          ...
          {workspaces.length > 1 && (
            <label>
              <span className="sr-only">切換工作空間</span>
              <select
                value={workspaceId}
                aria-label="切換工作空間"
                onChange={(event) => {
          ...
          <label>
            <span className="sr-only">切換專案</span>
            <select
              value={project.id}
              aria-label="切換專案"
              onChange={(event) => {
      ...
      <main className="main">
          ...
          <div className="topbar-actions">
            ...
            <div className="avatar" title={currentUser.name}>
              {currentUser.avatarUrl ? (
                ...
                currentUser.avatarEmoji || initials
              )}
            </div>
          </div>
        </header>
        {showSettings ? (
          ...
          <GanttBoard
            projectId={project.id}
            initialTasks={projectRecords.tasks}
          ...
          <CollaborativeEditor
            ...
            user={currentUser}
            editable={currentUser.role !== "VIEWER"}
            onCreateSubpage={requestCreateDocument}
            onCreateDatabase={requestCreateDatabase}
            onIconChange={(icon) =>
              setDocuments((current) =>
                current.map((item) => (item.id === active.id ? { ...item, icon } : item)),
              )
            }
          ...
          <section className="document">
            <div className="empty">正在載入文件…</div>
          </section>
        ) : (
          <section className="document">
            <div className="empty">尚無文件</div>
          </section>
        )}
      </main>
      <DocumentTemplatePicker
        ...
        onCreate={(templateId, title) => {
          ...
          void createDocument(title, undefined, templateId);
        }}
      />
      <aside className="rightbar">
            ...
            {(Object.keys(moduleLabels) as WorkspaceModule[]).map((name) => (
              ...
              >
                ...
                <span className="module-count">
                  {name === "gantt"
                    ? projectRecords.tasks.filter(
                        (task) => task.startDate && task.dueDate,
                      ).length
                    : projectRecords[name].length}
                </span>
              </button>
            ))}
          </div>
        ...
        <section className="signal-panel">
          <div className="panel-label">狀態訊號</div>
          <div className="signal-row">
            <span>未解決議題</span>
            <strong className={activeIssues ? "warning" : "good"}>{activeIssues}</strong>
          </div>
          <div className="signal-row">
            <span>測試紀錄</span>
    ...
    <div className="app-dialog-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <footer>
          ...
          <button
            type="button"
            className={dialog.destructive ? "dialog-danger" : "dialog-primary"}
