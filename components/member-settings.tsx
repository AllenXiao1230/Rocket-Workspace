...
export function MemberSettings({
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/workspaces/${workspaceId}/accounts`, {
      ...
      body: JSON.stringify({
        ...
        teamGroup: form.teamGroup || undefined,
        jobTitle: form.jobTitle || undefined,
      }),
    });
    const result = await response.json();
    setCreating(false);
    if (!response.ok) return setNotice(result.error || "無法建立帳號");
    setForm({
      name: "",
      email: "",
      password: "",
    ...
    setNotice("帳號已建立；請安全地將初始密碼交給該成員。");
    await reload();
  }
  ...
  return (
    <section className="settings-members member-settings">
      ...
      {canManage && accountProvisioningEnabled && (
        <section className="member-create-card">
          <h3>建立登入帳號</h3>
          <form className="member-create-form" onSubmit={create}>
            {(
              ["name", "email", "password", "nickname", "teamGroup", "jobTitle"] as const
            ).map((key) => (
              <input
                key={key}
                type={
                ...
                minLength={key === "password" ? minimumPasswordLength : undefined}
                value={form[key]}
                ...
                placeholder={
                  key === "name"
                    ...
                    : key === "email"
                      ? "電子郵件"
                      : key === "password"
                        ? `初始密碼（至少 ${minimumPasswordLength} 個字元）`
                        : key === "nickname"
                          ? "暱稱"
                          : key === "teamGroup"
                            ? "所屬分組"
      ...
      {canManage && !accountProvisioningEnabled && (
        <p className="hint">網頁建立帳號已由安全設定停用。</p>
      )}
