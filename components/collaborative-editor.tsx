...
export function CollaborativeEditor({ projectId, document, user, editable, onCreateSubpage, onIconChange, onDelete }: { projectId: string; document: DocumentData; user: { name: string; role: string }; editable: boolean; onCreateSubpage?: (parentId: string) => void; onIconChange?: (icon: string) => void; onDelete?: () => void }) {
  const collaborationRoom = document.id.startsWith("notion-") ? `document-${document.id}-notion-markdown-v2` : `document-${document.id}`;
  const [status, setStatus] = useState(editable ? "正在連接協作服務…" : "檢視模式"); const [sourceMode, setSourceMode] = useState(false); const [sourceMarkdown, setSourceMarkdown] = useState(document.markdown || ""); const [externalChanged, setExternalChanged] = useState(false); const [iconPicker, setIconPicker] = useState(false); const [customIcon, setCustomIcon] = useState(document.icon || "📄"); const [slashMenu, setSlashMenu] = useState(false); const [slashPosition, setSlashPosition] = useState<FloatingPosition>({ x: 120, y: 180 }); const [tablePosition, setTablePosition] = useState<FloatingPosition | null>(null); const [contextMenu, setContextMenu] = useState<EditorContextMenu | null>(null); const [activeProvider, setActiveProvider] = useState<WebsocketProvider | null>(null); const [onlineMembers, setOnlineMembers] = useState(0); const [imageDropActive, setImageDropActive] = useState(false);
  const saveTimer = useRef<ReturnType<typeof setTimeout> | null>(null); const saving = useRef(false); const pending = useRef<SaveSnapshot | null>(null); const ydoc = useRef(new Y.Doc()).current; const provider = useRef<WebsocketProvider | null>(null); const editorRef = useRef<ReturnType<typeof useEditor>>(null); const imageInputRef = useRef<HTMLInputElement>(null);
  ...
  async function uploadLocalImages(files: File[]) {
    ...
    for (const [index, file] of images.entries()) {
      ...
      const data = new FormData(); data.append("documentId", document.id); data.append("file", file);
      try {
        const response = await fetch("/api/attachments", { method: "POST", body: data });
        const result = await response.json() as { id?: string; filename?: string; error?: string };
        if (!response.ok || !result.id) { setStatus(result.error || `圖片「${file.name}」上傳失敗`); continue; }
        editor.chain().focus().setImage({ src: `/api/attachments?id=${encodeURIComponent(result.id)}`, alt: result.filename || file.name }).run();
      } catch { setStatus(`圖片「${file.name}」上傳失敗，請檢查網路後重試`); }
    }
    setStatus(images.length > 1 ? `已插入 ${images.length} 張圖片，正在儲存` : "圖片已插入文件，正在儲存");
  }
  function openLocalImagePicker() { if (editable) imageInputRef.current?.click(); }
  function onLocalImagePicked(event: ChangeEvent<HTMLInputElement>) { const files = Array.from(event.target.files || []); event.target.value = ""; void uploadLocalImages(files); }
  const editor = useEditor({ extensions: [StarterKit.configure({ undoRedo: false }), Underline, Image.configure({ allowBase64: false, HTMLAttributes: { loading: "lazy" } }), Callout, SecureEmbed, Link.configure({ openOnClick: false, autolink: true, defaultProtocol: "https" }), Table.configure({ resizable: true }), TableRow, TableHeader, TableCell, TaskList, TaskItem.configure({ nested: true }), Markdown.configure({ markedOptions: { gfm: true } }), Collaboration.configure({ document: ydoc }), ...(activeProvider ? [CollaborationCaret.configure({ provider: activeProvider, user: { name: user.name, color: collaboratorColor(user.name) } })] : []), Placeholder.configure({ placeholder: "輸入 / 開始寫作，變更會即時同步給在線成員。" })], editable, editorProps: { attributes: { "aria-label": "協作文件內容" }, handleKeyDown: (view, event) => { if (!editable) return false; const { $from } = view.state.selection; if (event.key === "/" && $from.parent.type.name === "paragraph" && !$from.parent.textContent) { const coords = view.coordsAtPos(view.state.selection.from); event.preventDefault(); setSlashPosition({ x: coords.left, y: coords.bottom + 8 }); setSlashMenu(true); return true; } if (event.key === "Escape") { setSlashMenu(false); setContextMenu(null); } return false; }, handleDOMEvents: { contextmenu: (view, event) => { if (!editable) return false; event.preventDefault(); const $from = view.state.selection.$from; setContextMenu({ x: event.clientX, y: event.clientY, kind: tableAtSelection($from) ? "table" : view.state.selection.empty ? "block" : "selection" }); return true; }, dragenter: (_, event) => { if (!editable || !Array.from(event.dataTransfer?.files || []).some((file) => file.type.startsWith("image/"))) return false; event.preventDefault(); setImageDropActive(true); return true; }, dragover: (_, event) => { if (!editable || !Array.from(event.dataTransfer?.files || []).some((file) => file.type.startsWith("image/"))) return false; event.preventDefault(); setImageDropActive(true); return true; }, dragleave: () => { setImageDropActive(false); return false; }, drop: () => { setImageDropActive(false); return false; } }, handleDrop: (_, event, moved) => { if (moved || !editable) return false; const images = Array.from(event.dataTransfer?.files || []).filter((file) => file.type.startsWith("image/")); if (!images.length) return false; event.preventDefault(); void uploadLocalImages(images); return true; }, handlePaste: (_, event) => { if (!editable) return false; const images = Array.from(event.clipboardData?.files || []).filter((file) => file.type.startsWith("image/")); if (!images.length) return false; event.preventDefault(); void uploadLocalImages(images); return true; } }, onSelectionUpdate: ({ editor: current }) => { if (current.isActive("table")) { const coords = current.view.coordsAtPos(current.state.selection.from); setTablePosition({ x: coords.left, y: coords.bottom + 8 }); } else setTablePosition(null); }, onUpdate: ({ editor: current }) => queueSave({ content: current.getJSON() as Record<string, unknown>, markdown: current.getMarkdown() }) }, [activeProvider]);
  useEffect(() => { editorRef.current = editor; }, [editor]);
  useEffect(() => { let disposed = false; const check = async () => { try { const response = await fetch(`/api/documents/${document.id}/markdown`, { cache: "no-store" }); const result = await response.json() as { externalChanged?: boolean }; if (!disposed && response.ok) setExternalChanged(Boolean(result.externalChanged)); } catch { /* keep editing available while the file check is offline */ } }; void check(); const timer = window.setInterval(() => void check(), 15_000); return () => { disposed = true; window.clearInterval(timer); }; }, [document.id]);
  ...
  function insertImage() { const src = window.prompt("圖片 HTTPS 網址"); if (!src) return; if (!isSafeDocumentEmbedUrl(src)) return setStatus("圖片僅接受有效的 HTTPS 網址"); const alt = window.prompt("圖片替代文字（可留空）", "") || ""; editor?.chain().focus().setImage({ src, alt }).run(); }
  function insertEmbed() { const url = window.prompt("嵌入內容 HTTPS 網址（例如 YouTube、Figma）"); if (!url) return; if (!isSafeDocumentEmbedUrl(url)) return setStatus("嵌入內容僅接受有效的 HTTPS 網址"); const label = window.prompt("嵌入標題", "外部嵌入內容") || "外部嵌入內容"; editor?.chain().focus().insertContent({ type: "secureEmbed", attrs: { url, label } }).run(); }
  ...
  async function reloadFromFile() { const response = await fetch(`/api/documents/${document.id}/markdown`, { cache: "no-store" }); const result = await response.json(); if (!response.ok) return setStatus(result.error || "無法讀取 Markdown 檔案"); setSourceMarkdown(result.markdown); if (!editable) return setSourceMode(true); if (!window.confirm("要以檔案中的 Markdown 覆蓋目前編輯器內容嗎？系統會先保存目前版本。")) return setSourceMode(true); editor?.commands.setContent(result.markdown, { contentType: "markdown" }); setExternalChanged(false); setSourceMode(false); setStatus("已載入外部 Markdown，正在建立新的協作版本"); }
  async function previewThreeWayMerge() { const response = await fetch(`/api/documents/${document.id}/markdown`, { cache: "no-store" }); const result = await response.json() as { markdown?: string; baseMarkdown?: string; error?: string }; if (!response.ok || result.markdown === undefined) return setStatus(result.error || "無法讀取 Markdown 檔案"); const merged = mergeMarkdown(result.baseMarkdown || "", editor?.getMarkdown() || document.markdown || "", result.markdown); setSourceMarkdown(merged.merged); setSourceMode(true); setStatus(merged.summary); }
  async function keepOnlineVersion() { if (!editor || !editable) return; try { const response = await fetch(`/api/documents/${document.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: editor.getJSON(), markdown: editor.getMarkdown() }) }); if (!response.ok) throw new Error(); setExternalChanged(false); setStatus("已保留線上版本並覆寫外部 Markdown"); } catch { setStatus("無法覆寫外部 Markdown，請稍後再試"); } }
  ...
  function insertBlock(kind: "paragraph" | "heading" | "task" | "quote" | "callout" | "code" | "table" | "divider" | "image" | "uploadImage" | "embed") {
    ...
    else if (kind === "embed") insertEmbed();
  ...
  return <article className="document"><div className="document-title-row"><button className="document-icon" aria-label="變更頁面圖標" disabled={!editable} onClick={() => { setCustomIcon(document.icon || "📄"); setIconPicker(true); }}>{document.icon || "📄"}</button><input className="document-title" aria-label="文件標題" defaultValue={document.title} readOnly={!editable} onBlur={(event) => updateTitle(event.currentTarget.value)} /></div>
    ...
    {externalChanged && <aside className="markdown-conflict" role="status"><strong>偵測到專案資料夾中的 Markdown 已被外部修改</strong><span>線上協作內容尚未被覆寫。你可比較三方基線；若兩邊都有改動，系統會以衝突標記保留兩個版本。</span><div><button onClick={() => void previewThreeWayMerge()}>三方合併預覽</button><button onClick={() => void reloadFromFile()}>檢視並載入檔案</button>{editable && <button className="markdown-conflict-keep" onClick={() => void keepOnlineVersion()}>保留線上版本</button>}</div></aside>}
    {sourceMode ? <section className="markdown-source"><div><strong>Markdown 原始碼</strong><span>套用後會轉成可協作的區塊；不支援的語法會保留為一般文字。</span></div><textarea aria-label="Markdown 原始碼" value={sourceMarkdown} readOnly={!editable} onChange={(event) => setSourceMarkdown(event.target.value)} spellCheck={false} /> <footer>{editable && <button className="collab-primary" onClick={applySource}>套用 Markdown</button>}<button className="source-close" onClick={() => setSourceMode(false)}>關閉</button></footer></section> : <div className={imageDropActive ? "editor-content-shell image-drop-active" : "editor-content-shell"}>{imageDropActive && <div className="image-drop-hint">放開即可上傳並插入圖片</div>}<EditorContent editor={editor} /></div>}
    <p className="editor-status">{status}{onlineMembers ? ` · 線上 ${onlineMembers} 位` : ""}</p><DocumentWorkflowPanel documentId={document.id} projectId={projectId} canWrite={editable} /><DocumentCollaborationPanel documentId={document.id} canWrite={editable} /><DocumentAttachments documentId={document.id} canWrite={editable} canPurge={user.role === "OWNER" || user.role === "ADMIN"} onInsertImage={(attachment) => editor?.chain().focus().setImage({ src: `/api/attachments?id=${encodeURIComponent(attachment.id)}`, alt: attachment.filename }).run()} /></article>;
}
