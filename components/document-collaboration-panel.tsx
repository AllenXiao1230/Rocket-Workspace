...
export function DocumentCollaborationPanel({
  ...
  const [pendingAction, setPendingAction] = useState<
    ...
    | { type: "restore-revision"; revisionId: string }
    | null
  >(null);
  ...
  const load = useCallback(async () => {
    ...
    if (mode === "comments") setComments(data);
  ...
  async function addComment(parentId?: string) {
    ...
    setReplyBody("");
    setReplyTo(null);
    setMessage(parentId ? "已新增回覆，對方會收到通知" : "已留言");
    await load();
  }
  ...
  async function compare(revisionId: string) {
    const response = await fetch(
      `/api/documents/${documentId}/revisions/${revisionId}/diff`,
    );
    const result = await response.json();
    if (!response.ok) return setMessage(result.error || "無法比較版本");
    setDiff(result.lines);
  }
  function commentItem(comment: Comment, reply = false) {
    return (
      <article key={comment.id} className={comment.resolvedAt ? "resolved" : ""}>
        <div className="comment-meta">
          <strong>{comment.author.name}</strong>
          <time>{new Date(comment.createdAt).toLocaleString()}</time>
          {comment.resolvedAt && <span>已解決</span>}
        </div>
        <p>{comment.body}</p>
        {canWrite && (
          <div className="comment-actions">
            ...
            <button onClick={() => void updateComment(comment.id, !comment.resolvedAt)}>
              {comment.resolvedAt ? "重新開啟" : "標記解決"}
            </button>
            {comment.canManage && (
              <button
                className="danger"
                onClick={() =>
                  setPendingAction({ type: "delete-comment", commentId: comment.id })
                }
              >
                刪除
              </button>
        ...
        {!reply && comment.replies.length > 0 && (
          <div className="comment-replies">
            {comment.replies.map((item) => commentItem(item, true))}
          </div>
        )}
      </article>
    );
  ...
  return (
    <section className="document-collab">
      <div>
        <button
          className={mode === "comments" ? "active" : ""}
          onClick={() => setMode(mode === "comments" ? null : "comments")}
        >
          💬 留言
        </button>
        <button
        ...
        <div className="collab-panel">
          <div className="comment-list">
            {comments.length ? (
              ...
              <p className="hint">尚無留言。用留言保留工程決策與待確認事項。</p>
            )}
          </div>
          {canWrite ? (
            <>
              <textarea
                ...
                placeholder="留下留言；回覆會通知原留言者"
              />
              <button className="collab-primary" onClick={() => void addComment()}>
                新增留言
              </button>
            </>
          ) : (
            <p className="hint">你目前為檢視者，只能閱讀留言。</p>
          )}
        </div>
      )}
      {mode === "history" && (
        <div className="collab-panel revision-list">
          {revisions.length ? (
            revisions.map((revision) => (
              <article key={revision.id}>
                <div>
                  ...
                  <span>
                    {revision.author?.name || "未知使用者"} ·{" "}
                    {new Date(revision.createdAt).toLocaleString()}
                  </span>
                </div>
                <button onClick={() => void compare(revision.id)}>比較</button>
                {canWrite && (
                  <button
                    onClick={() =>
                      setPendingAction({
            ...
            <p className="hint">第一次儲存後，版本歷史會自動建立。</p>
          )}
          {diff && (
      ...
      {message && <span className="collab-notice">{message}</span>}
      {pendingAction && (
        <ConfirmDialog
          title={pendingAction.type === "delete-comment" ? "刪除留言？" : "還原此版本？"}
          description={
            pendingAction.type === "delete-comment"
              ? "其回覆也會一併移除。"
              : "目前內容會保留在版本紀錄中。"
          }
          confirmLabel={pendingAction.type === "delete-comment" ? "刪除留言" : "還原版本"}
          destructive={pendingAction.type === "delete-comment"}
          onCancel={() => setPendingAction(null)}
          onConfirm={() => {
            const action = pendingAction;
            setPendingAction(null);
            if (action.type === "delete-comment") void deleteComment(action.commentId);
            else void restore(action.revisionId);
          }}
        />
      )}
    </section>
