...
const task = (id: string, status: string, overrides: Partial<MyTask> = {}): MyTask => ({
  id,
  title: id,
  status,
  priority: 3,
...
describe("groupMyTasks", () => {
  ...
  it("把逾期任務與較高優先級排在前面", () => {
    const groups = groupMyTasks([
      task("none", "TODO", { priority: 1 }),
      task("future", "TODO", { dueDate: "2099-01-02", priority: 1 }),
      task("overdue", "TODO", { dueDate: "2000-01-02", priority: 5 }),
    ]);
