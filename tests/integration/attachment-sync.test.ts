...
import { attachmentBucket, DeleteObjectCommand, GetObjectCommand, objectStorage } from "@/lib/object-storage";
...
beforeAll(async () => {
  ...
  const user = await prisma.user.create({ data: { email: `attachment-${suffix}@example.test`, name: "Integration Test", passwordHash: "not-used" } });
  const workspace = await prisma.workspace.create({ data: { name: "Attachment Test", slug: `attachment-${suffix}` } });
  await prisma.membership.create({ data: { userId: user.id, workspaceId: workspace.id, role: WorkspaceRole.OWNER } });
  const project = await prisma.project.create({ data: { workspaceId: workspace.id, code: `AT-${suffix.slice(0, 6)}`, name: "Attachment Test" } });
  const document = await prisma.document.create({ data: { projectId: project.id, title: "Attachment Test" } });
  storageKey = `integration/${suffix}.txt`;
  const attachment = await prisma.$transaction(async (tx) => {
    const created = await tx.attachment.create({ data: { documentId: document.id, filename: "proof.txt", mimeType: "text/plain", size: 5, storageKey } });
...
afterAll(async () => {
  if (storageKey) await objectStorage.send(new DeleteObjectCommand({ Bucket: attachmentBucket, Key: storageKey })).catch(() => undefined);
...
describe("attachment sync worker", () => {
  it("persists the queued payload to object storage and marks the attachment ready", async () => {
    ...
    const [attachment, object] = await Promise.all([
      ...
      objectStorage.send(new GetObjectCommand({ Bucket: attachmentBucket, Key: storageKey })),
    ]);
    ...
    await expect(prisma.attachmentSyncJob.findUnique({ where: { attachmentId } })).resolves.toBeNull();
  });
});
