...
import { canWrite, databaseAccess, documentAccess, projectAccess } from "@/lib/permissions";
...
beforeAll(async () => {
  ...
  const [owner, admin, editor, viewer, outsider] = await Promise.all([
    ...
    prisma.user.create({ data: { email: `admin-${suffix}@example.test`, name: "Admin", passwordHash: "not-used" } }),
    prisma.user.create({ data: { email: `editor-${suffix}@example.test`, name: "Editor", passwordHash: "not-used" } }),
  ...
  const outsiderWorkspace = await prisma.workspace.create({ data: { name: "Other Access Test", slug: `other-access-${suffix}` } });
  ...
  const [database, document] = await Promise.all([
    ...
    prisma.membership.create({ data: { userId: admin.id, workspaceId: workspace.id, role: WorkspaceRole.ADMIN } }),
    prisma.membership.create({ data: { userId: editor.id, workspaceId: workspace.id, role: WorkspaceRole.EDITOR } }),
    prisma.membership.create({ data: { userId: viewer.id, workspaceId: workspace.id, role: WorkspaceRole.VIEWER } }),
    prisma.membership.create({ data: { userId: outsider.id, workspaceId: outsiderWorkspace.id, role: WorkspaceRole.OWNER } }),
  ]);
...
afterAll(async () => {
  ...
  if (outsiderWorkspaceId) await prisma.workspace.delete({ where: { id: outsiderWorkspaceId } }).catch(() => undefined);
  await prisma.user.deleteMany({ where: { id: { in: [ownerId, adminId, editorId, viewerId, outsiderId].filter(Boolean) } } }).catch(() => undefined);
...
describe("workspace access boundaries", () => {
  it("returns each member's role for project, document, and database access", async () => {
    ...
    const expectedRoles = [WorkspaceRole.OWNER, WorkspaceRole.ADMIN, WorkspaceRole.EDITOR, WorkspaceRole.VIEWER];
    const access = await Promise.all(memberIds.map(async (userId) => {
    ...
    expect(access.map((result) => result.project?.membership.role)).toEqual(expectedRoles);
    expect(access.map((result) => result.document?.membership.role)).toEqual(expectedRoles);
    expect(access.map((result) => result.database?.membership.role)).toEqual(expectedRoles);
    expect(access.every((result) => result.document?.document.id === documentId && result.database?.database.id === databaseId)).toBe(true);
