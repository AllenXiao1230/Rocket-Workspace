...
describe("database row server validation", () => {
  it("accepts typed values and rejects unknown or computed fields", () => {
    expect(
      validateRowValues(properties, { name: "馬達", count: 2, status: "待處理" }),
    ).toMatchObject({ issues: [], values: { name: "馬達", count: 2, status: "待處理" } });
    expect(
      validateRowValues(properties, { formula: "9", unknown: "x" }).issues.map(
        (item) => item.message,
      ),
    ).toEqual(
      expect.arrayContaining([
        expect.stringContaining("由系統計算"),
        expect.stringContaining("欄位不存在"),
      ]),
    );
  });
  ...
  it("rejects invalid typed values", () => {
    expect(
      validateRowValues(properties, { count: "2", status: "錯誤" }).issues,
    ).toHaveLength(2);
  });
    ...
    const sourceProperties = [
      {
        ...
        options: { databaseId: "parts" },
      },
    ];
    const targetDatabase = {
      id: "parts",
      workspaceId: "workspace-a",
      properties: [{ id: "cost", name: "成本", type: "NUMBER" as const, options: null }],
    };
    expect(
      validatePropertyReference(
        "RELATION",
        { databaseId: "other" },
        { workspaceId: "workspace-a", sourceProperties, targetDatabase },
      ),
    ).not.toHaveLength(0);
    expect(
      validatePropertyReference(
