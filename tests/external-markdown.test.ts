...
import { externalMarkdownTitle, isExternalMarkdownFilename, markdownToDocumentContent } from "@/lib/external-markdown";

describe("外部 Markdown 文件", () => {
  it("以檔案中繼資料或標題建立安全的文件標題與內容", () => {
    const markdown = "---\ntitle: \"飛行測試\"\n---\n# 摘要\n\n測試完成";
    expect(externalMarkdownTitle("ignored.md", markdown)).toBe("飛行測試");
    expect(markdownToDocumentContent(markdown)).toMatchObject({ type: "doc", content: [{ type: "heading", attrs: { level: 1 } }, { type: "paragraph" }] });
  });
