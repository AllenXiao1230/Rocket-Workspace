...
describe("external HTTPS fetch", () => {
  it("connects with the public address verified during DNS validation", async () => {
    ...
    requestMock.mockImplementation((_: URL, options: { lookup: (hostname: string, lookupOptions: object, callback: (error: Error | null, address: string, family: number) => void) => void }, callback: (response: EventEmitter) => void) => {
      const request = Object.assign(new EventEmitter(), { end: () => {
        ...
        const response = Object.assign(new EventEmitter(), { headers: {}, statusCode: 200, statusMessage: "OK" });
        callback(response);
        response.emit("end");
      }, write: vi.fn() });
      return request;
    ...
    const response = await fetchExternalUrl("https://public.example/webhook", "WEBHOOK", undefined, { method: "POST", body: "{}" });
