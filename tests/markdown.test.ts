...
describe("tiptapToMarkdown", () => {
  it("保留常用格式與安全表格跳脫", () => {
    const markdown = tiptapToMarkdown({ type: "doc", content: [
      { type: "heading", attrs: { level: 2 }, content: [{ type: "text", text: "測試計畫" }] },
      { type: "paragraph", content: [{ type: "text", text: "重要", marks: [{ type: "bold" }] }, { type: "text", text: "資料" }] },
      { type: "table", content: [{ type: "tableRow", content: [
        { type: "tableHeader", content: [{ type: "paragraph", content: [{ type: "text", text: "欄位" }] }] },
