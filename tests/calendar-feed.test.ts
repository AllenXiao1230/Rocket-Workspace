...
describe("iCalendar project feed", () => {
  it("uses a stable non-reversible token digest", () => {
    ...
    expect(hashCalendarFeedToken("calendar-token")).toBe(
      hashCalendarFeedToken("calendar-token"),
    );
  });

  it("exports dated tasks and test records as escaped all-day events", () => {
    const ics = buildProjectCalendarIcs({
      projectId: "project-a",
      projectName: "飛行, 測試",
      tasks: [
        {
          ...
          updatedAt: new Date("2026-08-01T01:02:03.000Z"),
        },
      ],
      testRecords: [
        {
          ...
          title: "發射台測試",
          outcome: "PASS",
          testDate: new Date("2026-08-13T00:00:00.000Z"),
          notes: "確認\n完成",
    ...
    expect(ics).toContain(
      "DESCRIPTION:Rocket Workspace 測試紀錄\\n結果：PASS\\n確認\\n完成",
    );
  });
