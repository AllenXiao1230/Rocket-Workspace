...
const schema = z.object({
  ...
  layout: z.nativeEnum(DatabaseViewLayout).optional(),
    ...
    data: {
      ...
      filter:
        parsed.data.filter === undefined
          ? undefined
          : parsed.data.filter === null
            ? Prisma.JsonNull
            : (parsed.data.filter as Prisma.InputJsonValue),
      sort:
...
export async function DELETE(
  ...
  if (!access || !canWrite(access.membership.role))
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const count = await prisma.databaseView.count({ where: { databaseId: id } });
  if (count <= 1)
    return NextResponse.json({ error: "資料庫至少要保留一個檢視" }, { status: 400 });
  const deleted = await prisma.databaseView.deleteMany({
    where: { id: viewId, databaseId: id },
  });
