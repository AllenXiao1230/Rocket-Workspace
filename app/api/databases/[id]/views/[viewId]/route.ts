...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; viewId: string }> }) {
  ...
  return NextResponse.json(await prisma.databaseView.update({ where: { id: viewId }, data: { ...parsed.data, config: parsed.data.config === undefined ? undefined : parsed.data.config === null ? Prisma.JsonNull : parsed.data.config as Prisma.InputJsonValue, filter: parsed.data.filter === undefined ? undefined : parsed.data.filter === null ? Prisma.JsonNull : parsed.data.filter as Prisma.InputJsonValue, sort: parsed.data.sort === undefined ? undefined : parsed.data.sort === null ? Prisma.JsonNull : parsed.data.sort as Prisma.InputJsonValue } }));
}
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string; viewId: string }> }) {
  ...
  const deleted = await prisma.databaseView.deleteMany({ where: { id: viewId, databaseId: id } }); if (!deleted.count) return NextResponse.json({ error: "Not found" }, { status: 404 }); return NextResponse.json({ ok: true });
}
