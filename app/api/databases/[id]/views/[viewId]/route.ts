...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; viewId: string }> }) {
  ...
  await prisma.auditEvent.create({ data: { userId: session.user.id, action: "database_view.updated", entity: "database_view", entityId: viewId, workspaceId: access.database.project.workspaceId, projectId: access.database.projectId, metadata: { databaseId: id, changedFields: Object.keys(parsed.data) } } });
