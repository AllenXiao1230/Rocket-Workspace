...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const properties = await prisma.databaseProperty.findMany({ where: { databaseId: id }, select: { id: true } }); if (properties.length !== parsed.data.propertyIds.length || properties.some((property) => !parsed.data.propertyIds.includes(property.id))) return NextResponse.json({ error: "欄位清單已變更，請重新整理後再試" }, { status: 409 });
