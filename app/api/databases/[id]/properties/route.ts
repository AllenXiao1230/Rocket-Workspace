...
const schema = z.object({
  ...
  type: z.nativeEnum(DatabasePropertyType).default("TEXT"),
  options: optionValue.optional(),
});
export async function POST(
  ...
  const issues = validatePropertyOptions(parsed.data.type, parsed.data.options);
  if (issues.length)
  ...
  const databaseId =
    ...
    typeof parsed.data.options.databaseId === "string"
      ? parsed.data.options.databaseId
      : "";
  ...
  const referenceIssues = validatePropertyReference(
    parsed.data.type,
    parsed.data.options,
    {
  ...
  await prisma.auditEvent.create({
    data: {
      ...
      action: "database_property.created",
