...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; propertyId: string }> }) {
  ...
  const options = parsed.data.options === undefined ? property.options : parsed.data.options;
  const issues = validatePropertyOptions(type, options);
  if (issues.length) return NextResponse.json({ error: issues[0].message }, { status: 400 });
  const databaseId = typeof options === "object" && options && !Array.isArray(options) && typeof options.databaseId === "string" ? options.databaseId : "";
