...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string; templateId: string }> }) {
  ...
  const deleted = await prisma.databaseTemplate.deleteMany({ where: { id: templateId, databaseId: id } }); if (!deleted.count) return NextResponse.json({ error: "Not found" }, { status: 404 }); return NextResponse.json({ ok: true });
}
