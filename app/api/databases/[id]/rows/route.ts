...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const max = await prisma.databaseRow.aggregate({ where: { databaseId: id, deletedAt: null }, _max: { position: true } }); const automated = await applyRowAutomations(id, AutomationTrigger.ROW_CREATED, values, session.user.id);
