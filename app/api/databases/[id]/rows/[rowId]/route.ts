...
export async function PATCH(
  ...
  const { updated, createdRows } = await prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.databaseRow.update({
      where: { id: rowId },
      data: { values: finalValidation.values as Prisma.InputJsonValue },
    });
    const createdRows = [];
...
export async function DELETE(
  ...
  const deleted = await prisma.databaseRow.updateMany({
    where: { id: rowId, databaseId: id, deletedAt: null },
