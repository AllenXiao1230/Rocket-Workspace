...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; rowId: string }> }) {
  ...
  const mergedValues = { ...(row.values as Record<string, unknown>), ...validation.values }; const automated = await applyRowAutomations(id, AutomationTrigger.ROW_UPDATED, mergedValues, session.user.id);
