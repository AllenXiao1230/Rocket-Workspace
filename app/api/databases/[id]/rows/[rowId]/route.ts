...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; rowId: string }> }) {
  ...
  const row = await prisma.databaseRow.findFirst({ where: { id: rowId, databaseId: id } }); if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
  ...
  const max = await prisma.databaseRow.aggregate({ where: { databaseId: id }, _max: { position: true } });
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string; rowId: string }> }) {
  ...
  const { id, rowId } = await params; const access = await databaseAccess(session.user.id, id);
  if (!access || !canWrite(access.membership.role)) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const deleted = await prisma.databaseRow.deleteMany({ where: { id: rowId, databaseId: id } }); if (!deleted.count) return NextResponse.json({ error: "Not found" }, { status: 404 }); return NextResponse.json({ ok: true });
}
