...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json(
    await prisma.requirement.findMany({
      ...
      include: {
        verifications: {
          include: { testRecord: { select: { id: true, title: true, outcome: true } } },
        },
      },
      orderBy: { key: "asc" },
    }),
...
export async function POST(
  ...
  if (!input.success)
    return NextResponse.json({ error: "需求資料不正確" }, { status: 400 });
  let requirement;
  try {
    requirement = await prisma.requirement.create({
      data: { projectId: id, ...input.data },
    });
  } catch {
    return NextResponse.json({ error: "需求代碼已存在" }, { status: 409 });
  }
