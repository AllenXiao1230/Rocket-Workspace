...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const documents = await prisma.document.findMany({ where: { projectId: id, deletedAt: null, OR: [{ title: { contains: q, mode: "insensitive" } }, { markdownBase: { contains: q, mode: "insensitive" } }] }, select: { id: true, title: true, updatedAt: true }, take: 20, orderBy: { updatedAt: "desc" } });
