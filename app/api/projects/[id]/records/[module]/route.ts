...
const pageSize = (value: string | null) =>
  Math.min(100, Math.max(1, Number(value) || 50));
const dateValue = z
  .string()
  .regex(/^\d{4}-\d{2}-\d{2}$/)
  .nullable()
...
export async function GET(
  ...
  const kind = moduleSchema.safeParse(module);
  ...
  return NextResponse.json({
    records: data.slice(0, take),
    nextCursor: data.length > take ? data[take - 1].id : null,
  });
}
export async function POST(
  ...
  const access = await projectAccess(session.user.id, id);
  const kind = moduleSchema.safeParse(module);
  if (!access || !kind.success || !canWrite(access.membership.role))
  ...
  if (kind.data === "tasks") {
    ...
    if (
      ...
      (input.data.startDate &&
        input.data.dueDate &&
        input.data.startDate > input.data.dueDate)
    )
      return NextResponse.json({ error: "任務日期不正確" }, { status: 400 });
    if (
      input.data.assigneeId &&
      !(await prisma.membership.findFirst({
        where: { userId: input.data.assigneeId, workspaceId: access.project.workspaceId },
      }))
    )
    ...
    if (
      ...
      !(await prisma.task.findFirst({
        where: { id: input.data.parentId, projectId: id, deletedAt: null },
      }))
    )
      return NextResponse.json({ error: "父任務不存在" }, { status: 400 });
    return NextResponse.json(
      await prisma.task.create({
        data: {
          ...
          dueDate: input.data.dueDate
            ...
            : null,
          recurrenceAnchor: input.data.recurrenceAnchor
            ? new Date(`${input.data.recurrenceAnchor}T00:00:00.000Z`)
            : null,
          slaDueAt: input.data.slaDueAt
    ...
    return NextResponse.json(
      await prisma.issue.create({
        data: {
          ...
          key: input.data.key || `ISSUE-${Date.now()}`,
    ...
    return NextResponse.json(
      await prisma.bomItem.create({
        data: {
          projectId: id,
          ...input.data,
          partNumber: input.data.partNumber || `PART-${Date.now()}`,
          unitCost: input.data.unitCost || null,
        },
      }),
      { status: 201 },
    );
  ...
  if (
    ...
    (input.data.planId &&
      !(await prisma.testPlan.findFirst({
        where: { id: input.data.planId, projectId: id },
      })))
  )
