...
export async function GET(_: Request, { params }: { params: Promise<{ id: string; module: string }> }) {
  ...
  const data = kind.data === "tasks" ? await prisma.task.findMany({ where: { projectId: id, deletedAt: null }, include: { assignee: { select: { id: true, name: true, email: true } }, dependencies: { where: { dependsOn: { deletedAt: null } }, include: { dependsOn: { select: { id: true, title: true, status: true } } } } }, orderBy: { createdAt: "desc" } }) : kind.data === "issues" ? await prisma.issue.findMany({ where: { projectId: id, deletedAt: null }, orderBy: { createdAt: "desc" } }) : kind.data === "bom" ? await prisma.bomItem.findMany({ where: { projectId: id, deletedAt: null }, orderBy: { createdAt: "desc" } }) : await prisma.testRecord.findMany({ where: { projectId: id, deletedAt: null }, orderBy: { createdAt: "desc" } });
