...
export async function GET(
  ...
  if (attachment.syncStatus !== "READY")
    return NextResponse.json({ error: "附件仍在同步中" }, { status: 409 });
  try {
    ...
    return NextResponse.json({ error: "附件資料無法讀取" }, { status: 404 });
  }
}
export async function POST(
  ...
  if (
    ...
    return NextResponse.json(
      { error: "附件檔案不正確、超過大小限制，或伺服器附件設定無效" },
      { status: 400 },
    );
  if (
    !(await readWorkspaceSettings(result.access.project.workspaceId)).security
      .attachmentsEnabled
  )
    return NextResponse.json({ error: "管理者已停用附件上傳" }, { status: 403 });
  const storageKey = `${id}/${module}-${recordId}/${crypto.randomUUID()}-${safe(file.name)}`;
  ...
  if (allowedMimeTypes.size && !allowedMimeTypes.has(upload.mimeType))
    return NextResponse.json({ error: "此附件類型不被允許" }, { status: 415 });
  const attachment = await prisma.$transaction(async (tx) => {
    ...
    await tx.auditEvent.create({
      data: {
        action: "record_attachment.upload_queued",
        entity: "attachment",
        entityId: created.id,
        userId: session.user.id,
        workspaceId: result.access.project.workspaceId,
        projectId: id,
        metadata: { module, recordId, filename: created.filename, size: created.size },
      },
    });
    return created;
  });
...
export async function DELETE(
  ...
  await prisma.auditEvent.create({
    data: {
      action: "record_attachment.trashed",
      entity: "attachment",
