...
export async function PATCH(_: Request, { params }: { params: Promise<{ id: string; module: string; batchId: string }> }) {
  ...
  await prisma.auditEvent.create({ data: { userId: session.user.id, action: `${module}.restored`, entity: module, entityId: batchId, workspaceId: access.project.workspaceId, projectId: id, metadata: { restoredCount: result.count } } });
