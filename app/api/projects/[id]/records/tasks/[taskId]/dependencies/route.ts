...
export async function PATCH(
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "前置任務資料不正確" }, { status: 400 });
  ...
  if (dependencyIds.includes(taskId))
    return NextResponse.json({ error: "任務不能依賴自己" }, { status: 400 });
  const available = await prisma.task.findMany({
    where: { id: { in: dependencyIds }, projectId, deletedAt: null },
    select: { id: true },
  });
  if (available.length !== dependencyIds.length)
    return NextResponse.json(
      { error: "前置任務必須位於同一專案且尚未刪除" },
      { status: 400 },
    );
  for (const dependencyId of dependencyIds) {
    if (await reachesTask(dependencyId, taskId, projectId))
      return NextResponse.json(
        { error: "這項設定會形成循環相依，未儲存" },
        { status: 400 },
      );
  }
  ...
  await prisma.$transaction([
    ...
    prisma.taskDependency.createMany({
      data: dependencyIds.map((dependsOnId) => ({ taskId, dependsOnId })),
    }),
  ]);
