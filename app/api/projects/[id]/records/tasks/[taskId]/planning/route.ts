...
export async function PATCH(
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "排程資料不正確" }, { status: 400 });
  const task = await prisma.task.findFirst({
    where: { id: taskId, projectId, deletedAt: null },
  });
  if (!task) return NextResponse.json({ error: "找不到任務" }, { status: 404 });
  const startDate =
    parsed.data.startDate === undefined ? task.startDate : toDate(parsed.data.startDate);
  const dueDate =
    parsed.data.dueDate === undefined ? task.dueDate : toDate(parsed.data.dueDate);
  const baselineStartDate =
    parsed.data.baselineStartDate === undefined
      ? task.baselineStartDate
      : toDate(parsed.data.baselineStartDate);
  const baselineDueDate =
    parsed.data.baselineDueDate === undefined
      ? task.baselineDueDate
      : toDate(parsed.data.baselineDueDate);
  if (startDate && dueDate && startDate > dueDate)
    return NextResponse.json({ error: "開始日期不可晚於結束日期" }, { status: 400 });
  if (baselineStartDate && baselineDueDate && baselineStartDate > baselineDueDate)
    return NextResponse.json({ error: "基線開始日期不可晚於結束日期" }, { status: 400 });
  const updated = await prisma.task.update({
    ...
    data: {
      ...
      baselineDueDate,
      estimatedHours:
        parsed.data.estimatedHours === undefined ? undefined : parsed.data.estimatedHours,
    },
    include: {
