...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await runTaskAutomation({ projectId: id, dueOnly: false, includeSlaAlerts: false });
  await prisma.auditEvent.create({ data: { userId: session.user.id, action: "task.recurrence_run", entity: "project", entityId: id, workspaceId: access.project.workspaceId, projectId: id, metadata: { created: result.recurrences, date: result.runDate } } });
