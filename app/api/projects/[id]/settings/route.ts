...
import { readBackupStatus, readBackupSettings, writeBackupSettings } from "@/lib/runtime-settings";
import { publicWorkspaceSettings, readWorkspaceSettings, workspaceSettingsData } from "@/lib/workspace-settings";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [current, account] = await Promise.all([readWorkspaceSettings(result.access.project.workspaceId), prisma.user.findUnique({ where: { id: result.session.user.id }, select: { isSystemAdmin: true } })]);
    const next = {
      security: input.data.security || current.security,
      ai: input.data.ai ? { ...input.data.ai, apiKey: input.data.ai.apiKey?.trim() || current.ai.apiKey } : current.ai,
      integrations: input.data.integrations ? { ...input.data.integrations, githubToken: input.data.integrations.githubToken?.trim() || current.integrations.githubToken, webhookSecret: input.data.integrations.webhookSecret?.trim() || current.integrations.webhookSecret } : current.integrations,
    };
    if (next.ai.enabled) {
      if (!next.ai.baseUrl || !next.ai.model || (next.ai.provider === "OPENAI_COMPATIBLE" && !next.ai.apiKey)) return NextResponse.json({ error: "啟用 AI 前必須填入服務網址、模型與必要 API 金鑰" }, { status: 422 });
      await validateExternalUrl(next.ai.baseUrl, "AI", next.ai.provider);
    }
    if (next.integrations.webhookEnabled) {
      if (!next.integrations.webhookUrl) return NextResponse.json({ error: "啟用 Webhook 前必須填入 HTTPS 網址" }, { status: 422 });
    ...
    if (input.data.backupIntervalHours !== undefined || input.data.backupRetentionDays !== undefined) {
      if (!account?.isSystemAdmin) return NextResponse.json({ error: "只有系統管理員可調整主機備份排程" }, { status: 403 });
    }
    ...
    const [workspace, project] = await prisma.$transaction([
      ...
      prisma.workspaceSettings.upsert({ where: { workspaceId: result.access.project.workspaceId }, update: settingsData, create: { workspaceId: result.access.project.workspaceId, ...settingsData } }),
    ]);
    if (input.data.backupIntervalHours !== undefined || input.data.backupRetentionDays !== undefined) {
    ...
    return NextResponse.json({ error: code === "P2002" ? "此專案代碼已被使用" : message.includes("WORKSPACE_SETTINGS_ENCRYPTION_KEY") ? "儲存整合密鑰前，請在 .env 設定至少 32 字元的 WORKSPACE_SETTINGS_ENCRYPTION_KEY 或 AUTH_SECRET，然後重啟服務" : "無法儲存設定" }, { status: code === "P2002" ? 409 : 500 });
  }
}
