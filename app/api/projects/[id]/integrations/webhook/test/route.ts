...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!access || !["OWNER", "ADMIN"].includes(access.membership.role))
    return NextResponse.json({ error: "只有管理員能送出 Webhook 測試" }, { status: 403 });
  const integration = (await readWorkspaceSettings(access.project.workspaceId))
    .integrations;
  if (!integration.webhookEnabled)
    return NextResponse.json({ error: "管理者尚未啟用 Webhook 整合" }, { status: 403 });
  try {
    const payload = JSON.stringify({
      event: "rocket_workspace.webhook_test",
      projectId: id,
      sentAt: new Date().toISOString(),
    });
    const signature = integration.webhookSecret
      ? createHmac("sha256", integration.webhookSecret).update(payload).digest("hex")
      : "";
    const response = await fetchExternalUrl(
      ...
      undefined,
      {
        ...
        headers: {
          ...
          "X-Rocket-Workspace-Event": "webhook_test",
          ...(signature ? { "X-Rocket-Workspace-Signature": `sha256=${signature}` } : {}),
        },
        body: payload,
