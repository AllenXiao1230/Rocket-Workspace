...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const url = new URL(integration.webhookUrl); if (!/^https?:$/.test(url.protocol)) throw new Error("Webhook 網址必須使用 http 或 https"); const payload = JSON.stringify({ event: "rocket_workspace.webhook_test", projectId: id, sentAt: new Date().toISOString() }); const signature = integration.webhookSecret ? createHmac("sha256", integration.webhookSecret).update(payload).digest("hex") : ""; const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", "X-Rocket-Workspace-Event": "webhook_test", ...(signature ? { "X-Rocket-Workspace-Signature": `sha256=${signature}` } : {}) }, body: payload, signal: AbortSignal.timeout(30_000) }); return NextResponse.json({ ok: response.ok, status: response.status }, { status: response.ok ? 200 : 502 }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? `Webhook 測試失敗：${error.message}` : "Webhook 測試失敗" }, { status: 502 }); }
}
