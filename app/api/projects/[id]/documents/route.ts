...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const document = await prisma.document.create({ data: { projectId: id, parentId: parsed.data.parentId ?? null, title: parsed.data.title, icon: parsed.data.icon || template?.icon || "📄", content: template?.content as Prisma.InputJsonValue || undefined, properties: template?.properties as Prisma.InputJsonValue || undefined, position: (max._max.position ?? -1) + 1 } });
