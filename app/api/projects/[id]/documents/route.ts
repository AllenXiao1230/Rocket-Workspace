...
const pageSize = (value: string | null) =>
  Math.min(200, Math.max(1, Number(value) || 100));
...
export async function POST(
  ...
  if (parsed.data.parentId) {
    const parent = await prisma.document.findFirst({
      where: { id: parsed.data.parentId, projectId: id, deletedAt: null },
    });
    if (!parent)
      return NextResponse.json({ error: "Invalid parent document" }, { status: 400 });
  }
  const template = parsed.data.templateId
    ? await prisma.documentTemplate.findFirst({
        where: { id: parsed.data.templateId, projectId: id },
      })
    : null;
  if (parsed.data.templateId && !template)
    return NextResponse.json({ error: "文件模板不存在" }, { status: 400 });
  const max = await prisma.document.aggregate({
    where: { projectId: id, parentId: parsed.data.parentId ?? null, deletedAt: null },
    _max: { position: true },
  });
  const document = await prisma.$transaction(async (tx) => {
    const created = await tx.document.create({
      data: {
        ...
        icon: parsed.data.icon || template?.icon || "📄",
        content: (template?.content as Prisma.InputJsonValue) || undefined,
        properties: (template?.properties as Prisma.InputJsonValue) || undefined,
        position: (max._max.position ?? -1) + 1,
      },
    });
    await enqueueDocumentSync(tx, created.id, DocumentSyncAction.WRITE);
    return created;
  });
