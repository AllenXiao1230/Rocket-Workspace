...
const databaseInclude = {
  ...
  rows: { orderBy: { position: "asc" as const } },
};
...
export async function POST(
  ...
  const parentDocumentId =
    typeof body.parentDocumentId === "string" && body.parentDocumentId.trim()
      ? body.parentDocumentId.trim()
      : null;
  if (parentDocumentId) {
    const parent = await prisma.document.findFirst({
      where: { id: parentDocumentId, projectId: id, deletedAt: null },
      select: { id: true },
    });
    if (!parent)
      return NextResponse.json({ error: "Parent document not found" }, { status: 400 });
  }
  ...
  await prisma.auditEvent.create({
    data: {
      ...
      projectId: id,
      metadata: { name: database.name, parentDocumentId },
    },
  });
