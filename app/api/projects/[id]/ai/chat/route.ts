...
function endpoint(baseUrl: string, provider: "OPENAI_COMPATIBLE" | "OLLAMA") { const url = new URL(baseUrl); if (!/^https?:$/.test(url.protocol)) throw new Error("AI 服務網址必須使用 http 或 https"); url.pathname = `${url.pathname.replace(/\/$/, "")}${provider === "OLLAMA" ? "/api/chat" : "/chat/completions"}`; return url; }
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ai = (await readRuntimeSettings()).ai;
