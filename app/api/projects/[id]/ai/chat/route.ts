...
async function endpoint(baseUrl: string, provider: "OPENAI_COMPATIBLE" | "OLLAMA") { const url = new URL(baseUrl); url.pathname = `${url.pathname.replace(/\/$/, "")}${provider === "OLLAMA" ? "/api/chat" : "/chat/completions"}`; return url; }
