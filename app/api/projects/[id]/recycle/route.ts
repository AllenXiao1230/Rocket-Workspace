...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const roots = documents.filter(
    (document) =>
      !document.parentId ||
      !documents.some((candidate) => candidate.id === document.parentId),
  );
  return NextResponse.json(
    roots.map((document) => ({
      ...document,
      descendantCount: document.deletionBatchId
        ? documents.filter(
            (candidate) => candidate.deletionBatchId === document.deletionBatchId,
          ).length - 1
        : 0,
    })),
  );
