...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const member = await prisma.$transaction(async (tx) => {
      const requester = await tx.membership.findFirst({ where: { userId: session.user.id, workspaceId: id, role: { in: ["OWNER", "ADMIN"] } } });
      if (!requester) throw new Error("FORBIDDEN");
      const user = await tx.user.findUnique({ where: { email: input.data.email.toLowerCase() } });
    ...
    if (message === "FORBIDDEN" || message === "OWNER_PROTECTED") return NextResponse.json({ error: "只有擁有者能建立、變更或移除擁有者角色，且工作空間至少要保留一位擁有者" }, { status: 403 });
    if (message === "USER_NOT_FOUND") return NextResponse.json({ error: "Create the user account before adding a membership" }, { status: 404 });
