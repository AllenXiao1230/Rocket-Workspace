...
const updateSchema = z.object({
  ...
  teamGroup: z.string().trim().min(1).max(60).nullable().optional(),
...
export async function PATCH(
  ...
  if (!input.success)
    return NextResponse.json({ error: "成員資料不正確" }, { status: 400 });
  try {
    const member = await prisma.$transaction(
      async (tx) => {
        const [requester, target] = await Promise.all([
          ...
          tx.membership.findFirst({ where: { id: memberId, workspaceId: id } }),
        ]);
        ...
        const ownerCount =
          target.role === "OWNER" && nextRole !== "OWNER"
            ...
            : 2;
        if (!canChangeMembershipRole(requester.role, target.role, nextRole, ownerCount))
        ...
        return {
          ...
          user: {
            ...
            avatarUrl: member.user.avatarKey
              ? `/api/account/avatar?userId=${encodeURIComponent(member.user.id)}&v=${encodeURIComponent(member.user.avatarKey.slice(-12))}`
              : null,
          },
        };
      },
    ...
    if (message === "NOT_FOUND")
      return NextResponse.json({ error: "找不到成員或沒有管理權限" }, { status: 404 });
    if (message === "OWNER_PROTECTED")
      return NextResponse.json(
        { error: "只有擁有者能變更擁有者角色，且工作空間至少要保留一位擁有者" },
        { status: 403 },
      );
    const code =
...
export async function DELETE(
  ...
  try {
      ...
      async (tx) => {
        ...
        const ownerCount =
          target.role === "OWNER"
            ? await tx.membership.count({ where: { workspaceId: id, role: "OWNER" } })
            : 2;
        if (!canChangeMembershipRole(requester.role, target.role, "VIEWER", ownerCount))
          throw new Error("OWNER_PROTECTED");
    ...
    if (message === "OWNER_PROTECTED")
      return NextResponse.json(
        { error: "只有擁有者能移除擁有者，且工作空間至少要保留一位擁有者" },
        { status: 403 },
      );
    const code =
      typeof error === "object" && error && "code" in error ? String(error.code) : "";
