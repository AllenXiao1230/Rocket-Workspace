...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await workspaceMembership(session.user.id, id)))
...
export async function POST(
  ...
  if (!membership || !["OWNER", "ADMIN"].includes(membership.role))
    return NextResponse.json({ error: "只有擁有者或管理員能建立專案" }, { status: 403 });
  const input = projectSchema.safeParse(await request.json().catch(() => null));
  if (!input.success)
    return NextResponse.json(
      { error: input.error.issues[0]?.message || "專案資料不正確" },
      { status: 400 },
    );
  try {
    ...
    return NextResponse.json(
      { error: code === "P2002" ? "此專案代碼已被使用" : "無法建立專案" },
