...
const schema = z.object({ name: z.string().trim().min(1).max(80), email: z.string().email(), password: z.string().min(12).max(128), nickname: z.string().trim().min(1).max(40).optional(), teamGroup: z.string().trim().min(1).max(60).optional(), jobTitle: z.string().trim().min(1).max(80).optional(), role: z.nativeEnum(WorkspaceRole).default(WorkspaceRole.VIEWER) });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const member = await prisma.$transaction(async (tx) => {
    const user = await tx.user.create({ data: { name: parsed.data.name, email, passwordHash: await bcrypt.hash(parsed.data.password, 12), mustChangePassword: true } });
