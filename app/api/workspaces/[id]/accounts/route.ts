...
export async function POST(
  ...
  const requester = await prisma.membership.findFirst({
    where: { userId: session.user.id, workspaceId: id, role: { in: ["OWNER", "ADMIN"] } },
  });
  if (!requester)
  ...
  if (parsed.data.password.length < security.minimumPasswordLength)
    return NextResponse.json(
      { error: `初始密碼至少需要 ${security.minimumPasswordLength} 個字元` },
      { status: 400 },
    );
  if (requester.role !== "OWNER" && parsed.data.role === "OWNER")
    return NextResponse.json(
