...
export async function GET() {
  try {
    await Promise.all([prisma.$queryRaw`SELECT 1`, objectStorage.send(new HeadBucketCommand({ Bucket: attachmentBucket })), checkRedis(), checkCollaborationService(), checkSchedulerHeartbeat()]);
    return NextResponse.json({ status: "ok", checks: { database: "ok", objectStorage: "ok", redis: "ok", collaboration: "ok", scheduler: "ok" } }, { headers: { "Cache-Control": "no-store" } });
  } catch {
