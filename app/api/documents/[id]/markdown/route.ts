...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (request.nextUrl.searchParams.get("download") === "1") {
    if (!(await readWorkspaceSettings(access.document.project.workspaceId)).security.markdownDownloadEnabled) return NextResponse.json({ error: "管理者已停用 Markdown 下載" }, { status: 403 });
