...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (request.nextUrl.searchParams.get("download") === "1") return new NextResponse(snapshot.raw, { headers: { "Content-Type": "text/markdown; charset=utf-8", "Content-Disposition": `attachment; filename="${access.document.id}.md"`, ETag: `\"${snapshot.hash}\"` } });
  return NextResponse.json({ markdown: snapshot.markdown, hash: snapshot.hash, modifiedAt: snapshot.modifiedAt }, { headers: { ETag: `\"${snapshot.hash}\"` } });
}
