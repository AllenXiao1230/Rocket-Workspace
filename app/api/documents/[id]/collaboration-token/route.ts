...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canWrite(access.membership.role) || (access.document.lockedById && access.document.lockedById !== session.user.id && !["OWNER", "ADMIN"].includes(access.membership.role))) return NextResponse.json({ readOnly: true, locked: Boolean(access.document.lockedById) });
