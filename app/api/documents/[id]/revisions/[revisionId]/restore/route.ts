...
export async function POST(_: Request, { params }: { params: Promise<{ id: string; revisionId: string }> }) {
  ...
  await writeDocumentMarkdown(document, revision.markdown || undefined); await prisma.auditEvent.create({ data: { userId: session.user.id, action: "document.revision_restored", entity: "document", entityId: id, metadata: { revisionId } } });
