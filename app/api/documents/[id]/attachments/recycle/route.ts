...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json(
    await prisma.attachment.findMany({
      where: { documentId: id, deletedAt: { not: null } },
      select: {
        id: true,
        filename: true,
        mimeType: true,
...
export async function PATCH(
  ...
  const restored = await prisma.attachment.updateMany({
    where: { id: attachmentId, documentId: id, deletedAt: { not: null } },
...
export async function DELETE(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.auditEvent.create({
      data: {
        action: "attachment.purge_queued",
        entity: "attachment",
        entityId: attachment.id,
        userId: session.user.id,
        workspaceId: access.document.project.workspaceId,
        projectId: access.document.projectId,
        metadata: { documentId: id, filename: attachment.filename, irreversible: true },
      },
    });
  });
