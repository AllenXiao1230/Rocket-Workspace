...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const document = await prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.document.update({ where: { id }, data: { title: parsed.data.title, icon: parsed.data.icon, content: parsed.data.content as Prisma.InputJsonValue | undefined } });
    await enqueueDocumentSync(tx, id, DocumentSyncAction.WRITE, parsed.data.markdown ?? null);
    return updated;
  });
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction(async (tx) => { await tx.document.updateMany({ where: { id: { in: removedIds } }, data: { deletedAt: new Date(), deletionBatchId } }); await Promise.all(removedIds.map((documentId) => enqueueDocumentSync(tx, documentId, DocumentSyncAction.DELETE))); });
