...
export async function POST(
  ...
  const parent = parsed.data.parentId
    ? await prisma.documentComment.findFirst({
        where: { id: parsed.data.parentId, documentId: id },
      })
    : null;
  ...
  await prisma.auditEvent.create({
    data: {
      ...
      action: "document_comment.created",
      entity: "document_comment",
      entityId: comment.id,
      workspaceId: access.document.project.workspaceId,
      projectId: access.document.projectId,
      metadata: { documentId: id, parentId: comment.parentId },
    },
  });
  return NextResponse.json(
    { ...comment, isAuthor: true, canManage: true },
    { status: 201 },
  );
}
