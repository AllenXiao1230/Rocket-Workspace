...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; commentId: string }> }) {
  ...
  await prisma.auditEvent.create({ data: { userId: session.user.id, action: parsed.data.resolved ? "document_comment.resolved" : "document_comment.reopened", entity: "document_comment", entityId: comment.id, workspaceId: access.document.project.workspaceId, projectId: access.document.projectId, metadata: { documentId: id } } });
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string; commentId: string }> }) {
  ...
  await prisma.auditEvent.create({ data: { userId: session.user.id, action: "document_comment.deleted", entity: "document_comment", entityId: comment.id, workspaceId: access.document.project.workspaceId, projectId: access.document.projectId, metadata: { documentId: id, authorId: comment.authorId } } });
