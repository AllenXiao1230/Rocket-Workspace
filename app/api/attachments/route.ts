...
const allowedMimeTypes = new Set(
  (process.env.ALLOWED_ATTACHMENT_MIME_TYPES || "")
    .split(",")
    .map((type) => type.trim())
    .filter(Boolean),
);
...
export async function GET(request: Request) {
  ...
  if (!attachmentId)
    return NextResponse.json({ error: "documentId or id required" }, { status: 400 });
  ...
  if (!access) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  if (attachment.syncStatus !== "READY")
    return NextResponse.json(
      { error: "Attachment is still synchronizing" },
      { status: 409 },
    );

  try {
    const object = await objectStorage.send(
      new GetObjectCommand({ Bucket: attachmentBucket, Key: attachment.storageKey }),
    );
    if (!object.Body)
...
export async function POST(request: Request) {
  ...
  if (!Number.isFinite(maxAttachmentBytes) || file.size > maxAttachmentBytes)
    return NextResponse.json(
      {
        error: `File exceeds the ${Math.floor(maxAttachmentBytes / 1024 / 1024)} MB upload limit`,
      },
      { status: 413 },
    );
  const access = await documentAccess(userId, documentId);
  ...
  if (
    !(await readWorkspaceSettings(access.document.project.workspaceId)).security
      .attachmentsEnabled
  )
    return NextResponse.json({ error: "管理者已停用附件上傳" }, { status: 403 });
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "無法驗證檔案內容" },
      { status: 415 },
    );
  }
  if (allowedMimeTypes.size && !allowedMimeTypes.has(upload.mimeType))
  ...
  const attachment = await prisma.$transaction(async (tx) => {
    ...
    await tx.auditEvent.create({
      data: {
        action: "attachment.upload_queued",
        entity: "attachment",
        entityId: created.id,
        userId,
        workspaceId: access.document.project.workspaceId,
        projectId: access.document.projectId,
        metadata: { documentId, filename: created.filename, size: created.size },
      },
    });
    return created;
  });
  ...
  return NextResponse.json(synced || attachment, {
    status: synced?.syncStatus === "READY" ? 201 : 202,
  });
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Could not remove attachment. Please try again." },
      { status: 502 },
    );
  }
}
