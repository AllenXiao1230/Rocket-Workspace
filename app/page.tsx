...
export default async function Home({ searchParams }: { searchParams: Promise<{ workspace?: string; project?: string; document?: string; task?: string }> }) {
  ...
  const project = await prisma.project.findFirst({
      ...
      include: {
        ...
        databases: { select: { id: true, name: true, icon: true, parentDocumentId: true }, orderBy: { createdAt: "asc" } },
      },
    });
  if (!project) redirect("/");
  ...
  return <WorkspaceShell user={{ id: session.user.id, name: account?.name || session.user.name || session.user.email || "Member", avatarEmoji: account?.avatarEmoji, avatarUrl: account?.avatarKey ? "/api/account/avatar" : null, role: membership.role }} workspace={workspace.name} workspaceId={workspace.id} workspaces={memberships.map((item) => item.workspace)} project={{ id: project.id, name: project.name, code: project.code }} projects={workspace.projects} documents={documents} nextDocumentCursor={nextDocumentCursor} initialActiveId={documents.some((document) => document.id === requestedDocumentId) ? requestedDocumentId : undefined} initialModule={task ? "tasks" : undefined} initialSelectedTaskId={task?.id} databases={project.databases.map((database) => ({ ...database, properties: [], rows: [], views: [], templates: [], automations: [] }))} records={{ tasks: [], issues: [], bom: [], tests: [] }} myTasks={[]} teamMembers={workspace.memberships.map((item) => ({ id: item.id, role: item.role, nickname: item.nickname, teamGroup: item.teamGroup, jobTitle: item.jobTitle, user: item.user }))} />;
}
