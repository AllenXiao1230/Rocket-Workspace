...
export function mergeMarkdown(base: string, local: string, remote: string): MarkdownMergeResult {
  if (local === remote) return { merged: local, conflict: false, summary: "兩個版本內容相同。" };
  if (local === base) return { merged: remote, conflict: false, summary: "僅專案資料夾版本有變更，已安全套用。" };
  if (remote === base) return { merged: local, conflict: false, summary: "僅線上版本有變更，已安全保留。" };
  return {
    merged: ["<<<<<<< 線上協作版本", local, "=======", remote, ">>>>>>> 專案資料夾版本"].join("\n"),
    conflict: true,
