...
function externalFilename(properties: unknown) {
  if (
    ...
    !("externalMarkdownFilename" in properties)
  )
    return null;
  const filename = String(
    (properties as Record<string, unknown>).externalMarkdownFilename || "",
  );
  return filename.endsWith(".md") && filename === path.basename(filename)
    ? filename
    : null;
}
export function documentMarkdownPath(
  document: Pick<StoredDocument, "id" | "title" | "properties">,
) {
  return path.join(
    root,
    "documents",
    externalFilename(document.properties) || `${document.id}.md`,
  );
}
...
export async function writeDocumentMarkdown(document: StoredDocument, markdown?: string) {
  ...
  const target = documentMarkdownPath(document);
  const temporary = `${target}.${process.pid}.${randomUUID()}.tmp`;
  await writeFile(temporary, rendered, "utf8");
  await rename(temporary, target);
}
export async function deleteDocumentMarkdown(
  document: Pick<StoredDocument, "id" | "title" | "properties">,
) {
  try {
    await unlink(documentMarkdownPath(document));
  } catch (error: unknown) {
    if (
...
export async function readDocumentMarkdownSnapshot(
  document: Pick<StoredDocument, "id" | "title" | "properties">,
...
export async function readDocumentMarkdown(
  document: Pick<StoredDocument, "id" | "title" | "properties">,
) {
