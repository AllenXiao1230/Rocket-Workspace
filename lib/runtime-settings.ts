...
const backupStatusRoot =
  process.env.BACKUP_STATUS_DIR || path.join(process.cwd(), "backups", "status");
...
function positive(value: number, fallback: number, max: number, min = 1) {
...
function defaults(): RuntimeSettings {
  return {
    backup: {
      intervalHours: positive(defaultInterval, 24, 720),
      retentionDays: positive(defaultRetention, 14, 3650),
    },
    security: {
      ...
      accountProvisioningEnabled: enabled(
        process.env.ACCOUNT_PROVISIONING_ENABLED,
        false,
      ),
      forcePasswordChangeOnNewAccount: enabled(
        process.env.FORCE_PASSWORD_CHANGE_ON_NEW_ACCOUNT,
        false,
      ),
      minimumPasswordLength: positive(
        Number(process.env.MINIMUM_PASSWORD_LENGTH || 12),
        12,
        128,
        8,
      ),
      ...
      loginWindowMinutes: positive(
        Number(process.env.LOGIN_WINDOW_MINUTES || 15),
        15,
        1440,
      ),
    },
  ...
  return {
    backup: {
      intervalHours: positive(
        Number(values.BACKUP_INTERVAL_HOURS),
        fallback.backup.intervalHours,
        720,
      ),
      retentionDays: positive(
        Number(values.BACKUP_RETENTION_DAYS),
...
export async function readBackupSettings() {
  return (await readRuntimeSettings()).backup;
}
export async function readSecuritySettings() {
  return (await readRuntimeSettings()).security;
}
export async function writeBackupSettings(backup: BackupSettings) {
  const settings = await readRuntimeSettings();
  await writeRuntimeSettings({ ...settings, backup });
}

export async function readBackupStatus() {
  const read = async (name: string) => {
    try {
      return (await readFile(path.join(backupStatusRoot, name), "utf8")).trim() || null;
    } catch {
      return null;
    }
  };
  return {
    lastSuccess: await read("last-success.txt"),
    lastFailure: await read("last-failure.txt"),
  };
}
