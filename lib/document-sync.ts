...
export async function processDocumentSyncJobs(limit = 100) {
  const jobs = await prisma.documentSyncJob.findMany({
  ...
  for (const job of jobs) {
    try {
      if (job.action === DocumentSyncAction.DELETE || job.document.deletedAt)
        await deleteDocumentMarkdown(job.document);
      else {
      ...
      await prisma.documentSyncJob.update({
        where: { id: job.id },
        data: {
          attempts: { increment: 1 },
          lastError:
            error instanceof Error ? error.message.slice(0, 1_000) : "Unknown sync error",
        },
      });
      failed += 1;
    }
