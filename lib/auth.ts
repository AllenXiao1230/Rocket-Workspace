  ...
  providers: [
    Credentials({
      ...
      authorize: async (raw) => {
        ...
        const security = user
          ? await readEffectiveSecuritySettings(user.id)
          : await readSecuritySettings();
        if (
          security.loginRateLimitEnabled &&
          !(await loginAllowed(email, security.loginMaxAttempts))
        )
          return null;
        if (!user || !(await bcrypt.compare(parsed.data.password, user.passwordHash))) {
          if (security.loginRateLimitEnabled)
            await failedLogin(email, security.loginWindowMinutes);
          return null;
        }
