...
export function nextRecurrenceOccurrence(rule: string, from: Date) {
  const match = /^FREQ=(DAILY|WEEKLY|MONTHLY);INTERVAL=(\d{1,3})$/.exec(
...
export const isRecurrenceRule = (rule: string | null | undefined) =>
  Boolean(rule && nextRecurrenceOccurrence(rule, new Date("2026-01-01T00:00:00.000Z")));

type AutomationOptions = {
  now?: Date;
  projectId?: string;
  dueOnly?: boolean;
  includeSlaAlerts?: boolean;
};
...
export async function runTaskAutomation({
  ...
  for (const task of sources) {
    ...
    const occurrence = task.recurrenceRule
      ? nextRecurrenceOccurrence(task.recurrenceRule, anchor)
      : null;
    ...
    try {
      await prisma.task.create({
        data: {
          ...
          recurrenceAnchor: occurrence,
          recurrenceSourceId,
          startDate: task.startDate
            ? new Date(task.startDate.getTime() + offset)
            : occurrence,
    ...
    await prisma.task.updateMany({
      where: { id: task.id, recurrenceProcessedAt: null },
      data: { recurrenceProcessedAt: now },
    });
  }
      ...
      try {
        await prisma.notification.create({
          data: {
            ...
            title: `SLA 提醒：${task.title}`,
            body: `任務 SLA 到期日：${task.slaDueAt?.toLocaleDateString("zh-TW")} · ${task.project.name}`,
            href: `/?project=${task.projectId}`,
            deduplicationKey: `sla:${task.id}:${dayKey(now)}`,
          },
        });
        alerts += 1;
      } catch (error) {
