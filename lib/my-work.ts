...
export function dayStart(value: Date) { return new Date(value.getFullYear(), value.getMonth(), value.getDate()).getTime(); }

export function groupMyTasks(tasks: MyTask[]): WorkGroups {
  const now = dayStart(new Date());
  const sort = (left: MyTask, right: MyTask) => {
    const leftDue = dueTime(left.dueDate); const rightDue = dueTime(right.dueDate);
    const leftOverdue = leftDue !== null && leftDue < now; const rightOverdue = rightDue !== null && rightDue < now;
  ...
  return {
    pending: open.filter((task) => task.status === "BACKLOG" || task.status === "TODO").sort(sort),
...
export function taskDueLabel(value: MyTask["dueDate"]) {
  ...
  const today = dayStart(new Date()); const delta = Math.round((due - today) / 86_400_000);
  if (delta < 0) return { text: "已逾期", state: "overdue" };
  if (delta === 0) return { text: "今天到期", state: "today" };
  if (delta === 1) return { text: "明天到期", state: "soon" };
  return { text: new Intl.DateTimeFormat("zh-TW", { month: "numeric", day: "numeric" }).format(new Date(value!)), state: "future" };
}
