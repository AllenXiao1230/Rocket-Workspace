...
function detectedMimeType(payload: Uint8Array) {
  if (
    hasSignature(payload, [0x52, 0x49, 0x46, 0x46]) &&
    hasSignature(payload, [0x57, 0x45, 0x42, 0x50], 8)
  )
    return "image/webp";
  return signatures.find((item) => hasSignature(payload, item.signature, item.offset))
    ?.mimeType;
}

function isKnownExecutable(payload: Uint8Array) {
  return (
    hasSignature(payload, [0x4d, 0x5a]) || hasSignature(payload, [0x7f, 0x45, 0x4c, 0x46])
  );
}
