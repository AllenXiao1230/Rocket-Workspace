...
function allowedOllamaHosts() {
  return new Set(
    (process.env.OLLAMA_ALLOWED_HOSTS || "ollama")
      .split(",")
...
async function resolveExternalDestination(
  ...
  try {
    ...
    throw new Error(`${kind === "AI" ? "AI 服務" : "Webhook"}網址格式不正確`);
  }
  if (url.username || url.password || url.hash)
    throw new Error("外部服務網址不可包含帳號、密碼或片段");
  const isOllama = kind === "AI" && provider === "OLLAMA";
  const host = url.hostname.toLowerCase();
  if (isOllama && url.protocol === "http:" && allowedOllamaHosts().has(host))
    return { url };
  if (url.protocol !== "https:")
    throw new Error(
      isOllama
        ? "Ollama 僅能使用已允許的內部 HTTP 主機，其他服務必須使用 HTTPS"
        : "外部服務網址必須使用 HTTPS",
    );
  ...
  try {
    ...
    if (error instanceof Error && error.message === "blocked")
      throw new Error("外部服務網址不可指向本機或私有網路位址");
...
export async function fetchExternalUrl(
  raw: string | URL,
  kind: ServiceKind,
  provider: "OPENAI_COMPATIBLE" | "OLLAMA" | undefined,
  options: ExternalFetchOptions,
) {
  const destination = await resolveExternalDestination(raw.toString(), kind, provider);
  if (!destination.address || destination.url.protocol === "http:")
    return fetch(destination.url, options);
    ...
    const request = httpsRequest(
      destination.url,
      {
        ...
        lookup: (hostname, _, callback) => {
          if (hostname.toLowerCase() !== destination.url.hostname.toLowerCase())
            return callback(new Error("外部服務主機不符"), "", 0);
