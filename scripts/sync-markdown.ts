...
async function main() {
  ...
  console.log(
    `Processed ${outbox.completed} document sync jobs (${outbox.failed} failed); created ${results.filter(Boolean).length} missing Markdown documents; preserved ${results.filter((result) => !result).length} existing files.`,
  );
}
