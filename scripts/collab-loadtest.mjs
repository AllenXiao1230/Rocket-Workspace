...
const urls = (process.env.COLLAB_URLS || "")
  .split(",")
  .map((value) => value.trim().replace(/\/$/, ""))
  .filter(Boolean);
const clients = Math.max(
  2,
  Math.min(250, Number(process.env.COLLAB_LOADTEST_CLIENTS || 36)),
);
const timeoutMs = Math.max(
  5_000,
  Math.min(120_000, Number(process.env.COLLAB_LOADTEST_TIMEOUT_MS || 30_000)),
);
const maxP95Ms = Math.max(
  100,
  Math.min(120_000, Number(process.env.COLLAB_LOADTEST_MAX_P95_MS || 3_000)),
);
...
async function until(predicate, label) {
  ...
  throw new Error(`${label}（逾時 ${timeoutMs}ms）`);
}
...
try {
  for (let index = 0; index < clients; index += 1) {
    ...
    nodesUsed.add(url);
    const token = await new SignJWT({ documentId: `loadtest-${runId}` })
      .setProtectedHeader({ alg: "HS256" })
      .setSubject(`loadtest-client-${index}`)
      .setIssuedAt()
    ...
    map.observe((event) => {
      for (const key of event.keysChanged) {
        ...
        observed.add(`${index}:${key}`);
        latencies.push(Date.now() - value.sentAt);
      }
    });
    instances.push({ index, doc, provider, map, synced: false });
    provider.on("sync", (synced) => {
      if (synced) instances[index].synced = true;
    });
  }

  await until(
    () => instances.every((instance) => instance.synced || instance.provider.synced),
    "所有客戶端完成 Yjs 初始同步",
  );
  const connectedMs = Date.now() - startedAt;
  for (const instance of instances)
    instance.map.set(`writer-${instance.index}`, {
      writer: instance.index,
      sentAt: Date.now(),
    });
  await until(
    () => instances.every((instance) => instance.map.size === clients),
    "所有節點都收到全部跨客戶端更新",
  );
  ...
  const summary = {
    result: p95 <= maxP95Ms ? "PASS" : "FAIL",
    runId,
    nodes: [...nodesUsed],
    clients,
    expectedRemoteDeliveries: expectedDeliveries,
    observedRemoteDeliveries: observed.size,
    connectionMs: connectedMs,
    propagation: { p50Ms: p50, p95Ms: p95, maxMs: max },
    threshold: { maxP95Ms },
