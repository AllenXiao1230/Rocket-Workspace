...
const websocketRequire = createRequire(
  new URL("../../node_modules/y-websocket/package.json", import.meta.url),
);
...
const allowedOrigins = new Set(
  (process.env.COLLABORATION_ALLOWED_ORIGINS || process.env.NEXTAUTH_URL || "")
    .split(",")
    .map((origin) => origin.trim().replace(/\/$/, ""))
    .filter(Boolean),
);
const replicationEnabled =
  process.env.COLLABORATION_REDIS_ENABLED !== "false" && Boolean(process.env.REDIS_URL);
const replicaId = process.env.COLLABORATION_INSTANCE_ID || randomUUID();
const streamLimit = Math.max(
  1000,
  Number(process.env.COLLABORATION_REDIS_STREAM_MAXLEN || 10000),
);
...
async function configureRedisReplication() {
  ...
  try {
    ...
    publisher.on("error", (error) =>
      console.error("Collaboration Redis publisher error", error.message),
    );
    subscriber.on("error", (error) =>
      console.error("Collaboration Redis subscriber error", error.message),
    );
    ...
    console.error(
      "Collaboration Redis replication unavailable; using this instance only",
...
function publish(channel, payload) {
  if (!publisher) return;
  void publisher
    .publish(channel, JSON.stringify({ replicaId, ...payload }))
    .catch((error) => console.error("Collaboration Redis publish error", error.message));
}
...
function applyRemoteUpdate(raw) {
  try {
    ...
    if (document)
      Y.applyUpdate(document, Buffer.from(message.update, "base64"), redisOrigin);
  } catch (error) {
    console.error(
      "Collaboration Redis update error",
      error instanceof Error ? error.message : error,
    );
  }
}
...
function applyRemoteAwareness(raw) {
  try {
    ...
    console.error(
      "Collaboration Redis awareness error",
      error instanceof Error ? error.message : error,
    );
  }
}
...
setContentInitializor(async (document) => {
  if (!publisher) return;
  try {
    const entries = await publisher.xRange(streamKey(document.name), "-", "+", {
      COUNT: streamLimit,
    });
    for (const entry of entries) {
      const update = entry.message.update;
      if (typeof update === "string")
        Y.applyUpdate(document, Buffer.from(update, "base64"), replayOrigin);
    }
  } catch (error) {
    console.error(
      `Unable to replay collaboration stream for ${document.name}`,
      error instanceof Error ? error.message : error,
    );
  }
});
...
function configureSharedDocument(room) {
  ...
  document.on("update", (update, origin) => {
    ...
    void publisher
      ...
      .catch((error) => console.error("Collaboration Redis stream error", error.message));
  ...
  document.awareness.on("update", ({ added, updated, removed }, origin) => {
    ...
    if (clients.length)
      publish(awarenessChannel, {
        room,
        update: Buffer.from(
          awarenessProtocol.encodeAwarenessUpdate(document.awareness, clients),
        ).toString("base64"),
      });
  });
  return document;
...
const server = createServer((_, response) => {
  ...
  response.end("Rocket Workspace collaboration server");
});
  ...
  try {
    ...
    wss.handleUpgrade(request, socket, head, (ws) => {
      ...
      const remaining = Math.max(
        1_000,
        (payload.exp || Math.floor(Date.now() / 1_000) + 600) * 1_000 - Date.now(),
      );
      const expiryTimer = setTimeout(
        () => ws.close(4001, "Collaboration token expired"),
        remaining,
      );
