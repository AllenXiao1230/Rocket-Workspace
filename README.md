...
- **一致的互動操作**：文件屬性新增／更新、連結／圖片／嵌入網址輸入、外部 Markdown 覆寫確認、工時與測試證據新增、資料庫檢視／模板／自動化建立，以及資料庫的匯入回復與列／欄位／檢視／模板／自動化／資料庫刪除、文件與專案模組的新增／移至回收桶、文件留言刪除、版本還原、附件刪除、移除成員與停用日曆訂閱使用應用程式內對話框；開啟時焦點會移入對話框並圈選，關閉後返回原控制項，支援 Escape。文件與編輯器選單可用方向鍵、Home、End 與 Escape 操作。
